...
\begin{abstract}
자율적 경제 에이전트와 대규모 언어 모델(LLM) 애플리케이션의 등장은 보안과 경제적 지속가능성을 유지하면서 대규모로 운영될 수 있는 분산화된 발견 및 검증 인프라에 대한 긴급한 필요를 만들어냈습니다. 이 종합적인 논문은 Solana 블록체인 위에 구축된 온체인 레지스트리 시스템인 AEA 네트워크(Autonomous Economic Agent Network)를 제시하며, AI 에이전트와 Model Context Protocol(MCP) 서버의 안전하고 확장 가능하며 경제적으로 인센티브화된 등록을 가능하게 합니다.
