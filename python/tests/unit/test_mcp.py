...
class TestMcpServerRegistryInterface:
    ...
    async def test_get_server_not_found(self):
        ...
        result = await self.mcp_registry.get_server("nonexistent_server", self.owner_keypair.pubkey())
    ...
    async def test_get_server_interface(self):
        ...
        self.mock_client.get_account_info = AsyncMock(return_value=None)
        result = await self.mcp_registry.get_server("test_server", self.owner_keypair.pubkey())
        assert result is None
    ...
    async def test_list_servers_by_owner_empty(self):
        ...
        servers = await self.mcp_registry.list_servers_by_owner(self.owner_keypair.pubkey())
        assert servers == []
    ...
    async def test_ping_server_interface(self):
        ...
        with patch.object(self.mcp_registry, 'ping_server') as mock_ping:
            ...
            result = await self.mcp_registry.ping_server("test_server", self.owner_keypair.pubkey())
    ...
    async def test_error_handling_patterns(self):
        ...
        self.mock_client.get_account_info = AsyncMock(side_effect=Exception("Network error"))
        
        with pytest.raises((SolanaAIRegistriesError, McpServerNotFoundError, Exception)):
