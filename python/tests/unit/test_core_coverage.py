...
from solana_ai_registries.idl import (
    IdlAccount,
    IdlError,
    IdlInstruction,
    IDLLoader,
...
class TestClientBasics:
    ...
    def test_init_custom(self):
        ...
        client = SolanaAIRegistriesClient(
            rpc_url="https://custom-rpc.com", commitment=Commitment("finalized")
        )
    ...
    def test_instruction_building_methods(self):
        ...
        instruction = client.build_update_agent_instruction(
            agent_id="test-agent", owner=owner, name="Updated Name"
        )
        assert instruction is not None

        instruction = client.build_deregister_agent_instruction(
            agent_id="test-agent", owner=owner
        )
        assert instruction is not None
        ...
        instruction = client.build_deregister_mcp_server_instruction(
            server_id="test-server", owner=owner
        )
        assert instruction is not None
    ...
    def test_serialization_methods(self):
        ...
        data = client._encode_register_agent_data(
            agent_id="test-agent", name="Test Agent", description="Test description"
        )
...
class TestPaymentManagerBasics:
    ...
    def test_instruction_building_methods(self):
        ...
        mock_client.agent_program_id = PublicKey.from_string(
        ...
        instruction = manager._create_spl_transfer_instruction(
            source=pubkey, destination=pubkey, owner=pubkey, amount=1000000000
        )
        assert instruction is not None
...
class TestIdlLoaderBasics:
    ...
    def test_data_classes(self):
        ...
        instruction = IdlInstruction(
            name="test_instruction", accounts=[], args=[], discriminant=0
        )
        ...
        # Test IdlAccount
        account = IdlAccount(
            name="TestAccount", type={"kind": "struct", "fields": []}, discriminant=1
        )
        assert account.name == "TestAccount"
        assert account.discriminant == 1

        # Test IdlType
        idl_type = IdlType(name="TestType", type={"kind": "enum", "variants": []})
        assert idl_type.name == "TestType"

        # Test IdlError
        idl_error = IdlError(code=6000, name="TestError", msg="Test error message")
...
class TestErrorScenarios:
    ...
    def test_payment_error_handling(self):
        ...
        from solana_ai_registries.exceptions import InsufficientFundsError, PaymentError
