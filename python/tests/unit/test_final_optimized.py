...
class TestFinalOptimized:
    ...
    def test_client_correct_signatures(self):
        ...
        data = client._encode_update_agent_data(name="Updated Agent")
        assert isinstance(data, bytes)
    ...
    async def test_registries_correct_signatures(self):
        ...
        results = await agent_registry.search_agents(
            query="test", skills=["python"], limit=10
        )
        assert isinstance(results, list)

        results = await mcp_registry.search_servers(query="test", limit=5)
        assert isinstance(results, list)
    ...
    def test_payment_manager_comprehensive(self):
        ...
        instruction = manager._create_spl_transfer_instruction(
            source=payer, destination=provider, owner=payer, amount=1000000
        )
        assert instruction is not None
    ...
    def test_massive_method_calls(self):
        ...
        for i in range(5):
            client._encode_register_agent_data(
                f"agent{i}", f"Agent {i}", f"Description {i}"
            )
            client._encode_register_mcp_server_data(
                f"server{i}",
                f"Server {i}",
                f"Description {i}",
