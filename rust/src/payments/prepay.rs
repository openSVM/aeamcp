...
impl PrepaidPaymentClient {
    ...
    pub async fn create_prepaid_account<S: Signer>(
        ...
        if args.initial_deposit == 0 {
            return Err(SdkError::ValidationError("Initial deposit cannot be zero".to_string()));
        }
        ...
        let signature = self.send_and_confirm_transaction(owner, vec![instruction]).await?;
        
        Ok((prepaid_pda, PaymentResult {
            signature,
    ...
    pub async fn top_up<S: Signer>(
        ...
        if args.amount == 0 {
            return Err(SdkError::ValidationError("Top up amount cannot be zero".to_string()));
        }
        ...
        let instruction = self.create_top_up_instruction(
            &owner.pubkey(),
            &prepaid_pda,
            token_mint,
            args.amount,
    ...
    pub async fn spend<S: Signer>(
        ...
        if !account.is_active {
            return Err(SdkError::ValidationError("Prepaid account is not active".to_string()));
        }
    ...
    pub async fn get_prepaid_account(&self, account_pda: &Pubkey) -> SdkResult<PrepaidAccount> {
        ...
        PrepaidAccount::try_from_slice(&account.data[8..])
            .map_err(|e| SdkError::DeserializationError(format!("Failed to deserialize prepaid account: {}", e)))
    }
    ...
    pub async fn set_account_active<S: Signer>(
        ...
        let instruction = self.create_set_active_instruction(
            &owner.pubkey(),
            &prepaid_pda,
            active,
        )?;
        
        self.send_and_confirm_transaction(owner, vec![instruction]).await
    }
...
mod tests {
    ...
    fn test_prepaid_service_type_min_spend() {
        assert_eq!(PrepaidServiceType::AgentService.min_spend(), MIN_SERVICE_FEE);
        assert_eq!(PrepaidServiceType::ToolUsage.min_spend(), MIN_TOOL_FEE);
        assert_eq!(PrepaidServiceType::ResourceAccess.min_spend(), MIN_RESOURCE_FEE);
        assert_eq!(PrepaidServiceType::PromptExecution.min_spend(), MIN_PROMPT_FEE);
