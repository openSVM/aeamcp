...
impl IdlRegistry {
    ...
    pub fn instruction_names(&self, program: &str) -> Vec<&str> {
        ...
        idl.instructions.iter().map(|instr| instr.name.as_str()).collect()
    }
    ...
    pub fn validate(&self) -> SdkResult<()> {
        ...
        for instruction in &required_agent_instructions {
            if self.find_instruction("agent_registry", instruction).is_none() {
                return Err(SdkError::ValidationError(
                    format!("Missing required agent registry instruction: {}", instruction)
                ));
            }
        }
        ...
        for instruction in &required_mcp_instructions {
            if self.find_instruction("mcp_server_registry", instruction).is_none() {
                return Err(SdkError::ValidationError(
                    format!("Missing required MCP server registry instruction: {}", instruction)
                ));
            }
        }
