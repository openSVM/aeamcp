...
use solana_ai_registries::{
    SolanaAiRegistriesClient, AgentBuilder, AgentStatus, ServiceEndpoint, AgentSkill,
    agent::{AgentPatch, AgentRegistry, MAX_AGENT_ID_LEN, MAX_AGENT_NAME_LEN, MAX_AGENT_DESCRIPTION_LEN},
};
use solana_sdk::{
...
async fn test_02_register_minimal_agent() {
    ...
    let agent = AgentBuilder::new("minimal-agent", "M")
        .build()
        .unwrap();
...
fn test_09_too_many_service_endpoints_fails() {
    ...
    for i in 0..=3 { // MAX_SERVICE_ENDPOINTS is 3
...
fn test_11_invalid_service_endpoint_fails() {
    let result = AgentBuilder::new("valid-id", "Valid Name")
        .add_service_endpoint("", "http://endpoint.com", true); // Empty protocol
...
fn test_12_too_many_skills_fails() {
    ...
    for i in 0..=10 { // MAX_SKILLS is 10
...
fn test_16_agent_patch_update_endpoints() {
    let mut patch = AgentPatch::default();
    let endpoints = vec![
        ServiceEndpoint::new("https".to_string(), "https://secure.endpoint.com".to_string(), true).unwrap()
    ];
...
fn test_17_agent_patch_update_skills() {
    let mut patch = AgentPatch::default();
    let skills = vec![
        AgentSkill::new("new-skill".to_string(), "New Skill".to_string(), vec!["new".to_string()]).unwrap()
    ];
...
fn test_26_complex_agent_workflow() {
    ...
    let new_endpoints = vec![
        ServiceEndpoint::new("https".to_string(), "https://secure.endpoint.com".to_string(), true).unwrap(),
        ServiceEndpoint::new("ws".to_string(), "ws://websocket.endpoint.com".to_string(), false).unwrap(),
    ];
    update_patch.service_endpoints = Some(new_endpoints);
    
    let new_skills = vec![
        AgentSkill::new("advanced-skill".to_string(), "Advanced Skill".to_string(), vec!["v2".to_string(), "advanced".to_string()]).unwrap(),
    ];
    ...
    let activate_ix = AgentRegistry::update_status(&program_id, &owner, agent_id, AgentStatus::Active as u8);
    assert!(activate_ix.is_ok());
    
    let deactivate_ix = AgentRegistry::update_status(&program_id, &owner, agent_id, AgentStatus::Inactive as u8);
