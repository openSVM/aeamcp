...
TEST_F(PaymentsTest, PaymentMethodConversion) {
  ...
  EXPECT_EQ(Payments::payment_method_to_string(PaymentMethod::SvmaiToken),
            "SvmaiToken");
  ...
  EXPECT_EQ(Payments::string_to_payment_method("SvmaiToken"),
            PaymentMethod::SvmaiToken);

  EXPECT_THROW(Payments::string_to_payment_method("InvalidMethod"),
               std::invalid_argument);
}

TEST_F(PaymentsTest, PaymentStatusConversion) {
  EXPECT_EQ(Payments::payment_status_to_string(PaymentStatus::Pending),
            "Pending");
  EXPECT_EQ(Payments::payment_status_to_string(PaymentStatus::Completed),
            "Completed");
  EXPECT_EQ(Payments::payment_status_to_string(PaymentStatus::Failed),
            "Failed");
}

TEST_F(PaymentsTest, PaymentTypeConversion) {
  EXPECT_EQ(Payments::payment_type_to_string(PaymentType::Prepay), "Prepay");
  EXPECT_EQ(Payments::payment_type_to_string(PaymentType::PayAsYouGo),
            "PayAsYouGo");
  EXPECT_EQ(Payments::payment_type_to_string(PaymentType::Subscription),
            "Subscription");
