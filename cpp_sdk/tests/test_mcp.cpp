...
TEST_F(McpTest, ProtocolConversion) {
  ...
  EXPECT_THROW(Mcp::string_to_protocol("InvalidProtocol"),
...
TEST_F(McpTest, CapabilityConversion) {
  ...
  EXPECT_THROW(Mcp::string_to_capability("InvalidCapability"),
               std::invalid_argument);
}
...
TEST_F(McpTest, EndpointValidation) {
  // Valid HTTP endpoint
  EXPECT_NO_THROW(
      Mcp::validate_endpoint(McpProtocol::Http, "https://api.example.com"));

  // Valid WebSocket endpoint
  EXPECT_NO_THROW(
      Mcp::validate_endpoint(McpProtocol::WebSocket, "wss://api.example.com"));

  // Valid Stdio endpoint
  EXPECT_NO_THROW(
      Mcp::validate_endpoint(McpProtocol::Stdio, "python mcp_server.py"));

  // Invalid HTTP endpoint
  EXPECT_THROW(Mcp::validate_endpoint(McpProtocol::Http, "invalid-url"),
               std::invalid_argument);
