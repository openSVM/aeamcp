...
TEST_F(Base58Test, InvalidBase58Characters) {
    ...
    std::vector<std::string> invalid_strings = {
        ...
        "!@#$%^&*()", // Special characters
        "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz0", // Valid chars + invalid '0'
    };
...
TEST_F(Base58Test, InvalidLengthHandling) {
    ...
    std::vector<std::string> wrong_length_strings = {
        ...
        "111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111", // Too long
    };
...
TEST_F(Base58Test, EmptyStringHandling) {
    EXPECT_THROW(PublicKey empty_key{""}, SdkException) << "Should throw for empty string";
    EXPECT_THROW(Signature empty_sig{""}, SdkException) << "Should throw for empty string";
}
...
TEST_F(Base58Test, LeadingZerosHandling) {
    ...
    EXPECT_TRUE(base58_str.substr(0, 3) == "111") << "Should have leading 1's: " << base58_str;
...
TEST_F(Base58Test, NullPointerHandling) {
    ...
    EXPECT_THROW(PublicKey null_key{static_cast<const uint8_t*>(nullptr)}, SdkException) 
        << "Should throw for null pointer";
    EXPECT_THROW(Signature null_sig{static_cast<const uint8_t*>(nullptr)}, SdkException) 
        << "Should throw for null pointer";
}
...
TEST_F(Base58Test, FuzzTesting) {
    ...
    for (int i = 0; i < 10000; ++i) {
        ...
        if (size == 32) {
            ...
            // Should not be empty
            EXPECT_FALSE(base58_str.empty()) << "Base58 string should not be empty for iteration " << i;
            
            // Should only contain valid base58 characters
            std::string valid_chars = "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz";
...
TEST_F(Base58Test, PerformanceTest) {
    ...
    EXPECT_LT(encode_time.count(), 50000) << "Encoding 1000 keys should take < 50ms";
    EXPECT_LT(decode_time.count(), 50000) << "Decoding 1000 keys should take < 50ms";
    
    // Verify all keys round-tripped correctly
    for (size_t i = 0; i < test_keys.size(); ++i) {
        PublicKey original(test_keys[i].data());
        EXPECT_EQ(original, decoded_keys[i]) << "Performance test round-trip failed for key " << i;
    }
}
...
TEST_F(Base58Test, AlphabetEdgeCases) {
    ...
    EXPECT_TRUE(first_base58.back() == '2') << "Should end with '2': " << first_base58;
    ...
    EXPECT_TRUE(last_base58.back() == 'z') << "Should end with 'z': " << last_base58;
}