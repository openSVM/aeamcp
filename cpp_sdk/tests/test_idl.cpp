...
TEST_F(IdlTest, CppTypeMapping) {
    ...
    EXPECT_EQ(Idl::get_cpp_type_name(IdlType::PublicKey), "SolanaAiRegistries::PublicKey");
    EXPECT_EQ(Idl::get_cpp_type_name(IdlType::Bytes), "std::vector<uint8_t>");
    
    // Test with struct name
    EXPECT_EQ(Idl::get_cpp_type_name(IdlType::Struct, "TestStruct"), "TestStruct");
    
    // Test with enum name
    EXPECT_EQ(Idl::get_cpp_type_name(IdlType::Enum, std::nullopt, "TestEnum"), "TestEnum");
}
