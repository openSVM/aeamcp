...
TEST_F(AgentTest, CapabilityConversion) {
    EXPECT_EQ(Agent::capability_to_string(AgentCapability::TextGeneration), "TextGeneration");
    EXPECT_EQ(Agent::capability_to_string(AgentCapability::ImageGeneration), "ImageGeneration");
    EXPECT_EQ(Agent::capability_to_string(AgentCapability::CodeGeneration), "CodeGeneration");
    
    EXPECT_EQ(Agent::string_to_capability("TextGeneration"), AgentCapability::TextGeneration);
    EXPECT_EQ(Agent::string_to_capability("ImageGeneration"), AgentCapability::ImageGeneration);
    EXPECT_EQ(Agent::string_to_capability("CodeGeneration"), AgentCapability::CodeGeneration);
    
    EXPECT_THROW(Agent::string_to_capability("InvalidCapability"), std::invalid_argument);
}

TEST_F(AgentTest, PricingModelConversion) {
    EXPECT_EQ(Agent::pricing_model_to_string(PricingModel::PerRequest), "PerRequest");
    EXPECT_EQ(Agent::pricing_model_to_string(PricingModel::PerToken), "PerToken");
    EXPECT_EQ(Agent::pricing_model_to_string(PricingModel::Subscription), "Subscription");
    EXPECT_EQ(Agent::pricing_model_to_string(PricingModel::Free), "Free");
    
    EXPECT_EQ(Agent::string_to_pricing_model("PerRequest"), PricingModel::PerRequest);
    EXPECT_EQ(Agent::string_to_pricing_model("PerToken"), PricingModel::PerToken);
    
    EXPECT_THROW(Agent::string_to_pricing_model("InvalidModel"), std::invalid_argument);
}
...
TEST_F(AgentTest, SearchFilters) {
    ...
    filters.capabilities = {AgentCapability::TextGeneration, AgentCapability::CodeGeneration};
