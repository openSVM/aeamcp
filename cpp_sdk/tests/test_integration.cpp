...
protected:
    void SetUp() override {
        ...
        config.timeout = std::chrono::milliseconds{60000}; // Longer timeout for integration tests
...
TEST_F(IntegrationTest, TransactionBuilding) {
    ...
    EXPECT_NO_THROW({
        auto transaction_data = builder
            .set_payer(payer)
            .add_instruction(payer, accounts, data)
            .build();
...
TEST_F(IntegrationTest, BalanceQueries) {
    ...
    EXPECT_NO_THROW({
        BalanceInfo sol_balance = payments_->get_balance(account, PaymentMethod::Sol);
...
TEST_F(IntegrationTest, IdlOperations) {
    ...
    EXPECT_NO_THROW({
        ...
        EXPECT_TRUE(errors.empty()) << "IDL validation errors: " << 
