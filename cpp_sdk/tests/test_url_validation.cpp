...
TEST_F(UrlValidationTest, InvalidHttpUrls) {
    ...
    for (const auto& url : invalid_http_urls) {
        ...
        EXPECT_THROW(agent.validate_registration_params(params), std::invalid_argument) 
...
TEST_F(UrlValidationTest, InvalidWebSocketUrls) {
    ...
    for (const auto& url : invalid_ws_urls) {
        EXPECT_THROW(mcp.validate_endpoint(McpProtocol::WebSocket, url), std::invalid_argument) 
...
TEST_F(UrlValidationTest, McpProtocolValidation) {
    ...
    EXPECT_NO_THROW(mcp.validate_endpoint(McpProtocol::Http, "http://example.com"));
    EXPECT_NO_THROW(mcp.validate_endpoint(McpProtocol::Http, "https://example.com"));
    
    // Test HTTP protocol with invalid HTTP URLs
    EXPECT_THROW(mcp.validate_endpoint(McpProtocol::Http, "ws://example.com"), std::invalid_argument);
    EXPECT_THROW(mcp.validate_endpoint(McpProtocol::Http, "ftp://example.com"), std::invalid_argument);
    
    // Test WebSocket protocol with valid WebSocket URLs
    EXPECT_NO_THROW(mcp.validate_endpoint(McpProtocol::WebSocket, "ws://example.com"));
    EXPECT_NO_THROW(mcp.validate_endpoint(McpProtocol::WebSocket, "wss://example.com"));
    ...
    EXPECT_THROW(mcp.validate_endpoint(McpProtocol::WebSocket, "https://example.com"), std::invalid_argument);
    
    // Test Stdio protocol (any non-empty string should be valid)
    EXPECT_NO_THROW(mcp.validate_endpoint(McpProtocol::Stdio, "stdio"));
    EXPECT_NO_THROW(mcp.validate_endpoint(McpProtocol::Stdio, "pipe:///tmp/socket"));
    ...
    EXPECT_NO_THROW(mcp.validate_endpoint(McpProtocol::Custom, "custom://protocol"));
    EXPECT_NO_THROW(mcp.validate_endpoint(McpProtocol::Custom, "anything goes"));
    EXPECT_NO_THROW(mcp.validate_endpoint(McpProtocol::Custom, "tcp://localhost:1234"));
}
...
TEST_F(UrlValidationTest, EdgeCases) {
    ...
    // Test URLs with international domain names (should work with proper encoding)
...
TEST_F(UrlValidationTest, PerformanceTest) {
    ...
    for (int i = 0; i < 10000; ++i) {
        test_urls.push_back("https://example" + std::to_string(i) + ".com/api/v1/endpoint");
    }
    ...
    // Should validate 10,000 URLs in under 1 second
