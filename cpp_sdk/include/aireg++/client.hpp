...
struct ClientConfig {
    ...
    std::optional<std::string> custom_rpc_url;            ///< Custom RPC URL (overrides cluster)
    std::chrono::milliseconds timeout{30000};             ///< Request timeout in milliseconds
    std::optional<std::string> commitment = "confirmed";   ///< Transaction commitment level
...
public:
     ...
     */
    TransactionResult send_and_confirm_transaction(const std::vector<uint8_t>& transaction_data);
     ...
     * @throws TransactionException if transaction fails or times out
     */
    TransactionResult confirm_transaction(const Signature& signature, 
