...
struct PaymentInfo {
    ...
    std::optional<Signature> transaction_signature;  ///< Transaction signature if completed
...
struct PrepayParams {
    ...
    std::optional<PublicKey> token_mint;             ///< Custom token mint (if method is Custom)
    std::optional<std::string> memo;                 ///< Payment memo
...
struct PaymentSearchFilters {
    ...
    std::optional<std::chrono::system_clock::time_point> created_after; ///< Created after timestamp
    std::optional<std::chrono::system_clock::time_point> created_before; ///< Created before timestamp
...
public:
     ...
     */
    std::pair<PublicKey, Signature> create_prepayment(const PrepayParams& params,
     ...
     */
    std::pair<PublicKey, Signature> pay_as_you_go(const PayAsYouGoParams& params,
                                                  const std::vector<uint8_t>& payer_keypair);
     ...
     */
    std::pair<PublicKey, Signature> create_subscription(const SubscriptionParams& params,
                                                        const std::vector<uint8_t>& payer_keypair);
     ...
     */
    std::pair<PublicKey, Signature> start_stream(const StreamParams& params,
                                                 const std::vector<uint8_t>& payer_keypair);
     ...
     */
    std::vector<PaymentInfo> search_payments(const PaymentSearchFilters& filters = {},
                                           size_t limit = 100,
                                           size_t offset = 0);
