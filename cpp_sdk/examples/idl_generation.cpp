...
int main() {
    try {
        ...
        std::cout << "Bool -> " << Idl::get_cpp_type_name(IdlType::Bool) << std::endl;
        std::cout << "U64 -> " << Idl::get_cpp_type_name(IdlType::U64) << std::endl;
        std::cout << "String -> " << Idl::get_cpp_type_name(IdlType::String) << std::endl;
        std::cout << "PublicKey -> " << Idl::get_cpp_type_name(IdlType::PublicKey) << std::endl;
        
        // Test serialization sizes
        std::cout << "\n3. Testing serialization sizes..." << std::endl;
        auto bool_size = Idl::get_serialization_size(IdlType::Bool);
        std::cout << "Bool size: " << (bool_size ? std::to_string(*bool_size) : "variable") << " bytes" << std::endl;
        
        auto u64_size = Idl::get_serialization_size(IdlType::U64);
        std::cout << "U64 size: " << (u64_size ? std::to_string(*u64_size) : "variable") << " bytes" << std::endl;
        
        auto string_size = Idl::get_serialization_size(IdlType::String);
        std::cout << "String size: " << (string_size ? std::to_string(*string_size) : "variable") << " bytes" << std::endl;
        ...
        std::cout << "Generated header size: " << code.header_content.length() << " characters" << std::endl;
        std::cout << "Generated source size: " << code.source_content.length() << " characters" << std::endl;
        std::cout << "Dependencies: " << code.dependencies.size() << std::endl;
        
        std::cout << "\nIDL generation example completed successfully!" << std::endl;
