...
int main() {
    try {
        ...
        std::cout << "Connecting to " << cluster_to_url(config.cluster) << std::endl;
        
        // Create client
        Client client(config);
        std::cout << "Connected to Solana devnet: " << client.is_connected() << std::endl;
        ...
        std::cout << "System program ID: " << system_program.to_base58() << std::endl;
        ...
        if (account_info) {
            ...
            std::cout << "  - Owner: " << account_info->owner.to_base58() << std::endl;
            std::cout << "  - Executable: " << (account_info->executable ? "true" : "false") << std::endl;
            std::cout << "  - Data size: " << account_info->data.size() << " bytes" << std::endl;
        }
        
        // Get balance
        uint64_t balance = client.get_balance(system_program);
        std::cout << "System program balance: " << balance << " lamports" << std::endl;
        
        // Calculate rent exemption
        uint64_t rent_exemption = client.get_minimum_balance_for_rent_exemption(100);
        std::cout << "Rent exemption for 100 bytes: " << rent_exemption << " lamports" << std::endl;
        ...
        for (const auto& agent_info : agents) {
            std::cout << "  - " << agent_info.name << " (v" << agent_info.version << ")" << std::endl;
            std::cout << "    ID: " << agent_info.agent_id.to_base58() << std::endl;
            std::cout << "    Endpoint: " << agent_info.api_endpoint << std::endl;
            std::cout << "    Pricing: " << Agent::pricing_model_to_string(agent_info.pricing_model) << std::endl;
            if (agent_info.pricing_model == PricingModel::PerRequest) {
                std::cout << "    Price per request: " << agent_info.price_per_request << " lamports" << std::endl;
            }
        }
        ...
        std::cout << "Found " << servers.size() << " active MCP servers" << std::endl;
        
        for (const auto& server_info : servers) {
            std::cout << "  - " << server_info.name << " (v" << server_info.version << ")" << std::endl;
            std::cout << "    ID: " << server_info.server_id.to_base58() << std::endl;
            std::cout << "    Protocol: " << Mcp::protocol_to_string(server_info.protocol) << std::endl;
        ...
        auto transaction_data = builder
            .set_payer(payer)
            .add_instruction(payer, accounts, instruction_data)
            .build();
        
        std::cout << "Built transaction with " << transaction_data.size() << " bytes" << std::endl;
        
        uint64_t estimated_fee = builder.estimate_fee();
        std::cout << "Estimated transaction fee: " << estimated_fee << " lamports" << std::endl;
        
        // Test payment fee estimation
        std::cout << "\nTesting payment fee estimation..." << std::endl;
        uint64_t payment_fee = payments.estimate_payment_fee(PaymentMethod::Sol, 1000000);
        std::cout << "Estimated payment fee for 1 SOL: " << payment_fee << " lamports" << std::endl;
        
        // Test balance query
        BalanceInfo sol_balance = payments.get_balance(system_program, PaymentMethod::Sol);
        std::cout << "SOL balance query successful: " << sol_balance.balance << " lamports" << std::endl;
        ...
        std::cout << "Agent registry IDL: " << agent_idl.name << " v" << agent_idl.version << std::endl;
        std::cout << "  Instructions: " << agent_idl.instructions.size() << std::endl;
        ...
        std::cout << "MCP registry IDL: " << mcp_idl.name << " v" << mcp_idl.version << std::endl;
        ...
        std::cout << "SVMAI token IDL: " << token_idl.name << " v" << token_idl.version << std::endl;
        std::cout << "  Instructions: " << token_idl.instructions.size() << std::endl;
