...
int main() {
  try {
    ...
    std::cout << "SOL: "
              << Payments::payment_method_to_string(PaymentMethod::Sol)
              << std::endl;
    std::cout << "SVMAI Token: "
              << Payments::payment_method_to_string(PaymentMethod::SvmaiToken)
              << std::endl;
    std::cout << "USDC: "
              << Payments::payment_method_to_string(PaymentMethod::Usdc)
              << std::endl;

    // Test payment status conversions
    std::cout << "\n2. Testing payment status conversions..." << std::endl;
    std::cout << "Pending: "
              << Payments::payment_status_to_string(PaymentStatus::Pending)
              << std::endl;
    std::cout << "Completed: "
              << Payments::payment_status_to_string(PaymentStatus::Completed)
              << std::endl;
    std::cout << "Failed: "
              << Payments::payment_status_to_string(PaymentStatus::Failed)
              << std::endl;

    // Test fee estimation
    std::cout << "\n3. Testing fee estimation..." << std::endl;
    uint64_t fee = payments.estimate_payment_fee(PaymentMethod::Sol, 1000000);
    std::cout << "Estimated fee for 1 SOL payment: " << fee << " lamports"
              << std::endl;
