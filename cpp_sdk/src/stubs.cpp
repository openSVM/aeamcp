...
public:
  ...
  Impl(const ClientConfig &cfg) : config(cfg) {
    ...
    // Get RPC URL
    std::string rpc_url =
        cfg.custom_rpc_url.value_or(cluster_to_url(cfg.cluster));
...
TransactionResult Client::send_and_confirm_transaction(
...
TransactionBuilder::TransactionBuilder(Client &client)
    : pimpl_(std::make_unique<Impl>(client)) {}
...
std::vector<uint8_t> TransactionBuilder::build() {
  ...
  std::string blockhash =
      pimpl_->recent_blockhash.value_or(pimpl_->client.get_latest_blockhash());
  ...
  for (const auto &instruction : pimpl_->instructions) {
    ...
    if (account_indices.find(program_id_str) == account_indices.end()) {
      account_indices[program_id_str] =
          static_cast<uint8_t>(all_accounts.size());
  ...
  for (const auto &instruction : pimpl_->instructions) {
    ...
    // Data
    tx_data.insert(tx_data.end(), instruction.data.begin(),
...
TransactionBuilder::build_and_sign(const std::vector<uint8_t> &keypair_data) {
  ...
  std::vector<uint8_t> private_key(keypair_data.begin(),
  ...
  if (crypto_sign_detached(signature.data(), &sig_len, message.data(),
                           message.size(), private_key.data()) != 0) {
...
Signature Agent::update_agent(const PublicKey &agent_id,
...
std::vector<AgentInfo> Agent::search_agents(const AgentSearchFilters &filters,
                                            size_t limit, size_t offset) {
  // Mock implementation - return empty results
  return {};
}

std::vector<AgentInfo> Agent::get_agents_by_owner(const PublicKey &owner,
                                                  bool active_only) {
  // Mock implementation
  return {};
}

Signature Agent::deactivate_agent(const PublicKey &agent_id,
                                  const std::vector<uint8_t> &owner_keypair) {
  // Mock implementation
  return Signature();
}

Signature Agent::reactivate_agent(const PublicKey &agent_id,
                                  const std::vector<uint8_t> &owner_keypair) {
  // Mock implementation
  return Signature();
}

Signature Agent::delete_agent(const PublicKey &agent_id,
                              const std::vector<uint8_t> &owner_keypair) {
  // Mock implementation
  return Signature();
}
...
void Agent::validate_registration_params(
  ...
  if (!is_valid_http_url(params.api_endpoint)) {
    throw std::invalid_argument(
        "Agent API endpoint must be a valid HTTP/HTTPS URL");
  }
}
...
PublicKey Mcp::register_server(const McpRegistrationParams &params,
                               const std::vector<uint8_t> &owner_keypair) {
  validate_registration_params(params);
  // Mock implementation
  return PublicKey();
...
Signature Mcp::update_server(const PublicKey &server_id,
...
std::vector<McpServerInfo> Mcp::search_servers(const McpSearchFilters &filters,
                                               size_t limit, size_t offset) {
  // Mock implementation - return empty results
  return {};
}

std::vector<McpServerInfo> Mcp::get_servers_by_owner(const PublicKey &owner,
                                                     bool active_only) {
  // Mock implementation
  return {};
}
...
Signature Mcp::deactivate_server(const PublicKey &server_id,
                                 const std::vector<uint8_t> &owner_keypair) {
  // Mock implementation
  return Signature();
}

Signature Mcp::reactivate_server(const PublicKey &server_id,
                                 const std::vector<uint8_t> &owner_keypair) {
  // Mock implementation
  return Signature();
}

Signature Mcp::delete_server(const PublicKey &server_id,
                             const std::vector<uint8_t> &owner_keypair) {
  // Mock implementation
  return Signature();
}
...
Signature Payments::stop_stream(const PublicKey &stream_id,
                                const std::vector<uint8_t> &payer_keypair) {
  // Mock implementation
  return Signature();
}
...
BalanceInfo Payments::get_balance(const PublicKey &account,
                                  PaymentMethod method,
                                  std::optional<PublicKey> token_mint) {
  // Mock implementation
  BalanceInfo balance;
...
IdlDefinition Idl::parse_from_json(const std::string &json_content) {
  ...
  if (json_content.empty() || json_content.find('{') == std::string::npos ||
...
GeneratedCode Idl::generate_cpp_code(const IdlDefinition &idl,
...
std::string Idl::generate_header(const IdlDefinition &idl,
                                 const CodeGenOptions &options) {
  return generate_cpp_code(idl, options).header_content;
}

std::string Idl::generate_instruction_builders(const IdlDefinition &idl,
                                               const CodeGenOptions &options) {
  return "// Instruction builders for " + idl.name + "\n";
}

std::string Idl::generate_account_deserializers(const IdlDefinition &idl,
                                                const CodeGenOptions &options) {
...
std::string Idl::idl_type_to_string(IdlType type) {
  static const std::unordered_map<IdlType, std::string> type_map = {
      {IdlType::Bool, "Bool"},     {IdlType::U8, "U8"},
      {IdlType::I8, "I8"},         {IdlType::U16, "U16"},
      {IdlType::I16, "I16"},       {IdlType::U32, "U32"},
      {IdlType::I32, "I32"},       {IdlType::U64, "U64"},
      {IdlType::I64, "I64"},       {IdlType::U128, "U128"},
      {IdlType::I128, "I128"},     {IdlType::Bytes, "Bytes"},
      {IdlType::String, "String"}, {IdlType::PublicKey, "PublicKey"},
      {IdlType::Array, "Array"},   {IdlType::Vec, "Vec"},
      {IdlType::Option, "Option"}, {IdlType::Struct, "Struct"},
      {IdlType::Enum, "Enum"}};
...
IdlType Idl::string_to_idl_type(const std::string &type_str) {
  static const std::unordered_map<std::string, IdlType> string_map = {
      {"Bool", IdlType::Bool},     {"U8", IdlType::U8},
      {"I8", IdlType::I8},         {"U16", IdlType::U16},
      {"I16", IdlType::I16},       {"U32", IdlType::U32},
      {"I32", IdlType::I32},       {"U64", IdlType::U64},
      {"I64", IdlType::I64},       {"U128", IdlType::U128},
      {"I128", IdlType::I128},     {"Bytes", IdlType::Bytes},
      {"String", IdlType::String}, {"PublicKey", IdlType::PublicKey},
      {"Array", IdlType::Array},   {"Vec", IdlType::Vec},
      {"Option", IdlType::Option}, {"Struct", IdlType::Struct},
      {"Enum", IdlType::Enum}};
...
Idl::deserialize_account_data(const IdlStruct &struct_def,
                              const std::vector<uint8_t> &data) {
  // Mock implementation
