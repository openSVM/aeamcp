...
# Fixed End-to-End Testing for AEAMCP Devnet Deployment
# Tests core token-based functionality with proper account management
...
echo -e "${BLUE}================================================${NC}"
echo -e "${BLUE}  AEAMCP E2E Testing (Devnet) - Fixed${NC}"
echo -e "${BLUE}================================================${NC}"
echo ""
...
echo -e "${YELLOW}Funding test account with 0.05 SOL...${NC}"
solana transfer --from ~/.config/solana/id.json $USER_PUBKEY 0.05 --allow-unfunded-recipient
...
# Create token accounts with unique keypairs
echo -e "${YELLOW}Creating token accounts...${NC}"

# Create unique keypairs for each token account
solana-keygen new --no-bip39-passphrase --silent --outfile ./user-token-account.json
solana-keygen new --no-bip39-passphrase --silent --outfile ./deployer-token-account.json
solana-keygen new --no-bip39-passphrase --silent --outfile ./agent-fee-vault.json
solana-keygen new --no-bip39-passphrase --silent --outfile ./mcp-fee-vault.json
solana-keygen new --no-bip39-passphrase --silent --outfile ./stake-vault.json

# Create token accounts using the keypairs
USER_TOKEN_ACCOUNT=$(solana-keygen pubkey ./user-token-account.json)
DEPLOYER_TOKEN_ACCOUNT=$(solana-keygen pubkey ./deployer-token-account.json)
AGENT_FEE_VAULT=$(solana-keygen pubkey ./agent-fee-vault.json)
MCP_FEE_VAULT=$(solana-keygen pubkey ./mcp-fee-vault.json)
STAKE_VAULT=$(solana-keygen pubkey ./stake-vault.json)

# Create the actual token accounts
spl-token create-account $MINT_PUBKEY --account-keypair ./user-token-account.json --owner ./test-user.json
spl-token create-account $MINT_PUBKEY --account-keypair ./deployer-token-account.json
spl-token create-account $MINT_PUBKEY --account-keypair ./agent-fee-vault.json  
spl-token create-account $MINT_PUBKEY --account-keypair ./mcp-fee-vault.json
spl-token create-account $MINT_PUBKEY --account-keypair ./stake-vault.json
...
TEST_REPORT="e2e-test-report-fixed-$(date +%Y%m%d-%H%M%S).json"
...
cat > "$TEST_REPORT" << EOF
{
  ...
  "tests_completed": [
    ...
    "Unique token account creation and funding",
  ...
  "summary": {
    "total_fees_collected": "$(echo "$AGENT_FEE + $MCP_FEE" | bc) SVMAI",
    "total_staked": "$BRONZE_STAKE SVMAI",
    "remaining_user_balance": "$(spl-token balance --address $USER_TOKEN_ACCOUNT) SVMAI"
  }
}
EOF
...
rm -f ./test-user.json ./test-mint.json ./user-token-account.json ./deployer-token-account.json ./agent-fee-vault.json ./mcp-fee-vault.json ./stake-vault.json

echo ""
echo -e "${GREEN}✅ FIXED E2E TESTING COMPLETED SUCCESSFULLY!${NC}"
echo -e "${BLUE}================================================${NC}"
...
echo -e "${GREEN}✓ Proper account management${NC}"
echo -e "${BLUE}================================================${NC}"
echo ""
echo -e "${YELLOW}All token-based functionality working correctly!${NC}"
echo -e "${YELLOW}Ready for frontend integration and production deployment!${NC}"
